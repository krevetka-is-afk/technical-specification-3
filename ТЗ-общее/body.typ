...
3. Поиск и рекомендации через ML-гейт с keyword fallback при недоступности semantic ML.
...
2. Публикация событий в outbox table и доставка downstream consumers через REST/JSON API.
...
Для серверной части:
1. Backend на Python 3.13+ (модульный монолит на Django 5+ и DRF).
2. PostgreSQL с организацией схем по доменам.
3. Outbox REST/JSON delivery API для downstream-синхронизации.
4. Reference ML и graph services на FastAPI, Neo4j для графовой проекции.
...
3. Сетевые требования: HTTPS для пользователей и защищенные service tokens для downstream consumers.

== Требования к информационной и программной совместимости

=== Требования к информационным структурам и методам решения

Данные разделены по доменным Django apps. `web` является source of truth для users/projects/applications/deadlines/faculty mirror, а downstream-сервисы синхронизируются через outbox snapshot, poll, replay и ack. Доступ к системе осуществляется по HTTPS с использованием REST/JSON. Сервис импорта обеспечивает загрузку XLSX ЭПП и административный CSV-экспорт.
...
1. Локальная auth-модель с email verification; header-based SSO adapter допускается как integration layer.
2. Загрузка XLSX-файлов для импорта проектов.
3. CSV-экспорт данных для административной обработки.
4. Outbox delivery API, совместимый с выбранным стеком (Python, PostgreSQL).
...
Серверная часть: Python 3.12+, Django 5+, Django REST Framework; FastAPI для reference ML/graph services; библиотеки для работы с PostgreSQL, Neo4j и интеграциями с внешними сервисами.\
...
2. Использование session auth/DRF token и service tokens с хранением паролей/секретов вне кода (переменные окружения, vault).
...
#figure(
    table(
        ...
        [Техническое задание], [Утверждение требований], [Сбор исходных данных (курсы, ОП, роли), детализация функций, согласование архитектурного решения (Django + DRF + ML/graph reference services + outbox)], [до 10.01.2026],
...
4. Тесты безопасности (права доступа, email verification, service tokens, защита персональных данных).
