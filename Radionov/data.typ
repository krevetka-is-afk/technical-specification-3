#let data = (
  // Project code
  // Example: "01-01"
  // You can find it here: https://ru-ikt.ru/reestrpo
  code: "12.17",

  // Project name
  project_name: "Цифровой Ассистент Студента: клиентская часть",
  project_name_english: "Digital Student Assistant",

  // Students
  // Example: 
  //  (name: "А. В. Васюков", group: "БПИ235"),
  //  (name: "Л. И. Лившиц", group: "БПИ235"),
  // You can write 1 or more students
  students: (
    (name: "Н. Д. Радионов", group: "БПИ236"),
  ),

  // Agreed
  agreed_by_name: "А. А. Паринов",
  agreed_by_position: "Старший преподаватель: Департамент анализа данных и искусственного интеллекта,
НИУ ВШЭ",

  // Approved
  approved_by_name: "Н. А. Павлочев",
  approved_by_position: "Академический руководитель образовательной программы \"Программная инженерия\",\n старший преподаватель департамента программной инженерии",

  analogue_1: "ИПС ФЭН",
  analogue_2: "Ярмарка Элементов практической подготовки",
  analogue_3: "Текущий процесс на ФКН: Excel-таблицы и веб-формы",

  brief_characteristic_area-application_program:
    "Клиентская часть Цифрового Ассистента Студента представляет собой веб-интерфейс, предоставляющий студентам, заказчикам и модераторам единый доступ к каталогу проектов, фильтрам, рекомендациям и управлению заявками. Интерфейс работает в браузере без установки дополнительного программного обеспечения и реализует пользовательские сценарии, описанные в общем техническом задании.",
  
  operational_purpose: "Эксплуатация клиентской части осуществляется через веб-интерфейс в стандартных браузерах. Пользователи работают из локальных или удаленных сетей без установки дополнительных компонентов; все операции выполняются через защищенные соединения с серверной частью."
)
