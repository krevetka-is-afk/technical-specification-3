...
- предоставления REST/JSON и потоковых API по каталогам проектов, заявкам, модерации и рекомендациям;
- выполнения аутентификации через SSO edu.hse.ru и выдачи сессионных токенов/JWT;
- интеграции с брокером очередей и ML‑сервисом поиска/рекомендаций, публикации и приёма событий;
- импорта, очистки и нормализации данных из внешних таблиц; экспорта сведений в LMS/ЕЛК и форматы XLSX/CSV;
- обеспечения взаимодействия с клиентскими приложениями по согласованным контрактам.
Реализация серверной части — модульный монолит на Python (Django ASGI или FastAPI) с выделенным ML‑гейтом.
...
1. Импорт из Sheets/XLSX и нормализация данных.
...
#figure(
    table(
        ...
        [Разработка], [Минимум 1], [Базовые сущности и CRUD (проекты, заявки, пользователи), аутентификация SSO/JWT, каталог с фильтрами, импорт таблиц (async)], [Январь-Февраль 2026],
        ...
        [Тестирование и приемка], [ПМИ], [Функциональные, интеграционные и нагрузочные испытания (API, очередь, ML-гейт), устранение замечаний, подготовка релиза], [Апрель 2026],
    )
)
...
Аутентификация — процесс проверки подлинности пользователя или сервиса (SSO edu.hse.ru).
...
Потоковые API (SSE/Chunked) — интерфейсы передачи данных в режиме серверных событий или порционного вывода (chunked) для поиска и рекомендаций.
Брокер сообщений — программный компонент (Kafka, по согласованию RabbitMQ/Redis Streams) для обмена событиями между сервисами.
ML-гейт — выделенный сервис машинного обучения для семантического поиска и рекомендаций.
JWT — формат токена доступа, содержащий подтвержденные сервером атрибуты пользователя, подписанный секретом/ключом.
